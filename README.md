...
- The first registered account becomes **Admin**; all other accounts become **User**.
- To start with a clean slate, clear site data in your browser or remove the local storage key:
  ```
...
3. If using the fake backend, a "verification email" will appear as an alert with a link
...
3. If using the fake backend, a "reset password email" will appear as an alert with a link
...
| `src/app/_services/account.service.ts` | Login / logout / refresh / register / etc. |
| `src/app/_helpers/app.initializer.ts` | Tries to refresh on first app load |
| `src/app/_helpers/jwt.interceptor.ts` | Adds the `Authorization` header for API calls |
...
Role restrictions are applied using route data — for example, `/admin` requires `Role.Admin`.
